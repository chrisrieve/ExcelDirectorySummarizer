...
for file in data_files[:1]:
	...
	for sheet in xls.sheet_names:
        ...
        print 'type of sheet rows', type(sheet_rows)
        workbook = os.path.split(file)[1]
        print 'type of workbook', type(workbook)
        print 'workbook is', workbook
        wb_size = readablebytes.humanize_bytes(os.path.getsize(file))
        print 'type of wb size', type(wb_size)
        num_sheets = len(xls.sheet_names)
        print 'type of num_sheets', type(num_sheets)
        sheet_name = sheet
        print 'type of sheet name', type(sheet_name)
        c.execute('''INSERT INTO excel_sheets('Workbook','# sheets','Sheetname','rows','Workbook size') VALUES(?,?,?,?,?)''',(workbook, num_sheets, sheet_name,sheet_rows, wb_size))
